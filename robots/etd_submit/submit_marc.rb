    ...
    module EtdSubmit
      class SubmitMarc < Robots::DorRepo::EtdSubmit::Base
        ...
        def generate_marc(druid)
          ...
          statement_of_responsibility(marc, name, suffix, prefix, title)
        ...
        def statement_of_responsibility(marc, name, suffix, prefix, title)
        ...
        def parse_name(name_str)
          if /,/.match?(name_str)
            gen_suf = name_str.slice!(/Jr\.|Sr\.|III|IV|II/)

            name_str =~ /([^,\r\n]*),\s*(.*)/
            last = Regexp.last_match(1)
            first = Regexp.last_match(2)
            name_str = "#{first.strip} #{last.strip}"
            name_str.slice!(',')

            name_str << ", #{gen_suf.strip}" unless gen_suf.nil?
          end
          name_str
        end
      end
