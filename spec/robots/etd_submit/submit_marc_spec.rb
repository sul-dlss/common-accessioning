...
describe Robots::DorRepo::EtdSubmit::SubmitMarc do
  describe 'Exporting MARC' do
    ...
    describe '#process_items' do
      ...
      it "creates marc and saves it to a file in the day's working directory" do
        skip
        # marc_file = File.join(ROBOT_ROOT, 'tmp', Time.now.strftime('%Y%m%d'), "#{@mock_workitem.druid.gsub(':','_')}.marc")
        #         FileUtils.rm(marc_file) if File.exists(marc_file)
        #
        #         @robot.process_item(@mock_workitem)
        #         File.should be_file(marc_file)
      end
    end
